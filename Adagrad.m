...
samples= 1000;
learning_rate = 1.35;
beta = 0.9/samples % maybe this scales 
...
for i = 1:mu
   ...
   % teacher_1 = dot(teacher_w1, dataset);
    ...
    for j = 1:K
        ...
        gradeint_epsilon_studentweights_curr = gradeint_epsilon_studentweights + (gradeint_epsilon_studentweights).^2;
        final_gradeint = ((learning_rate/samples).*gradeint_epsilon_studentweights_curr)./(sqrt(gradeint_epsilon_studentweights) + epsilon);
       % velocities = beta * velocities + (1-beta)* ((learning_rate/samples).* gradeint_epsilon_studentweights');
        student_weights(j,:) =  student_weights(j,:) - final_gradeint;
